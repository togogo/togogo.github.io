...
*   W I T H   R A N D O M   B R A N C H
...
function preload() {
  ...
  //text
  randSource = loadStrings('./data/txt/random_source.txt');
...
function mousePressed() {
  ...
  //for debugging purposes...see what's the loaded strings look like.
  //print(randSource[0]);
  //print(randSource[0]);
  //print(randSource.length);
  //print(randSource[Math.floor(Math.random() * randSource.length)]);
}
...
//fake generation function that simply returns random advice from a text
function fakeGenStart(){
  
  if(model1isLive == true && window.getSelection().toString() != ""){
    print("Model 1 is trying to generate something...fake!");
    genArea1isWorking = true;
    genArea1.value(randSource[Math.floor(Math.random() * randSource.length)]);
    captureGenText(genArea1.value(), 1);
    genArea1isWorking = false;
    
  }
  
  if(model2isLive == true && window.getSelection().toString() != ""){
    print("Model 2 is trying to generate something...fake!");
    genArea2isWorking = true;
    genArea2.value(randSource[Math.floor(Math.random() * randSource.length)]);
    captureGenText(genArea2.value(), 2);
    genArea2isWorking = false;
  }
  
  if(model2isLive == true && window.getSelection().toString() != ""){
    print("Model 3 is trying to generate something...fake!");
    genArea3isWorking = true;
    genArea3.value(randSource[Math.floor(Math.random() * randSource.length)]);
